...
class Encoder(nn.Module):
    ...
    def forward(self, input, stmts, seq_lens):
        ...
        embedded_stmts = self.stmt_embedding(stmts)
        # embedded = torch.cat((embedded, embedded_stmts), dim=2)
        # seq_lens : batch_size * actual length
        packed = pack_padded_sequence(embedded, seq_lens, batch_first=True)
        stmt_packed = pack_padded_sequence(embedded_stmts, seq_lens, batch_first=True)
        output, hidden = self.lstm(packed)
        stmt_output, _ = self.stmt_lstm(stmt_packed)
        stmt_encoder_outputs, _ = pad_packed_sequence(stmt_output, batch_first=True)
        encoder_outputs, _ = pad_packed_sequence(output, batch_first=True)  # h dim = B x t_k x n (bi-directional lstm)
        encoder_outputs = encoder_outputs.contiguous()
        stmt_encoder_outputs =stmt_encoder_outputs.contiguous()

        encoder_feature = encoder_outputs.view(-1, 2*config.hidden_dim)  # B * t_k x 2*hidden_dim (3200*512)
        stmt_feature = stmt_encoder_outputs.view(-1, 2*config.hidden_dim)
        encoder_feature = self.W_h(encoder_feature)  # a linear, outsize:3200*512
        stmt_feature = self.stmt_W_h(stmt_feature)
...
class Attention(nn.Module):
    def __init__(self):
        ...
        self.stmt_v = nn.Linear(config.hidden_dim * 2, 1, bias=False)
